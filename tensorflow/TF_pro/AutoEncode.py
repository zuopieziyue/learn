# -*- coding: utf-8 -*-
"""
Created on Tue Dec 17 10:43:11 2019
...
#D(x) = (max-min)^2/12,0均值，方差为(min+max)/2 [-sqrt(6/(max+min)),sqrt(6/(max+min))]
...
class AutoEncoder(object):
	...
	def _initialize_weights(self):
		all_weight = dict()
		#初始化权重在合适范围内：
		#【初始化太小：在每层间传递更新较小，权重更新较小，初始化权重太大，容易发散】
...
def stanard_scale(X_train, X_test):
	preprocessor = StandardScaler().fit(X_train)
	X_train = preprocessor.transform(X_train)
	X_test = preprocessor.transform(X_test)
	return X_train, X_test

X_train, X_test = stanard_scale(mnist.train.images,mnist.test.images)
