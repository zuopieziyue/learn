...
def create_model(hps, vocab_size, num_classes):
	...
	with tf.variable_scope('cnn', initializer = cnn_init):
		...
		golbal_maxpooling = tf.reduce_max(conv1d, axis=[1])
...
with tf.Session() as sess:
	...
	for i in range(num_train_steps):
		...
		outputs_val = sess.run(
				[loss, accurary, train_op, global_step],
				feed_dict = {
						inputs: batch_inputs,
						outputs: batch_labels,
						keep_prob: train_keep_prob_value})
		loss_val, accurary_val, _, global_step_val = outputs_val
		if global_step_val % 20 == 0:
			tf.logging.info('step %5d, loss %3.3f, accurary %3.3f' % (global_step_val, loss_val, accurary_val))
