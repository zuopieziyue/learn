...
X_mask = sequence_mask(X, torch.tensor([1, 2]))
print(X_mask)
...
def train_seq2seq(net, data_iter, lr, num_epochs, tgt_vocab, device):
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % 10 == 0:
            print('epoch:', epoch + 1, 'loss: ', (metric[0] / metric[1]))
    print(f'loss {metric[0] / metric[1]:.3f}, {metric[1] / timer.stop():.1f}' f'token/sec on {str(device)}')
