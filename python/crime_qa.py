...
class CrimeQA:
    ...
    def load_embedding(self, embedding_path):
        ...
        for line in open(embedding_path):
            ...
            if count%10000 == 0:
                print(count, "loaded")
        print ("Loaded %s word embedding", count)
    ...
    def rep_sentencevector(self, setence, flag='seg'):
        if flag == "seg":
            wordlist = [i for i in setence.split(' ') if i]
        else:
            wordlist = self.seg_sent(setence)
        embedding = np.zeros(self.embedding_size)
        sent_len = 0
        for index, wd in enumerate(wordlist):
            if wd in embedding_dict:
                embedding += self.embedding_dict.get(wd)
                sent_len += 1
            else:
                continue
        return embedding/sent_len

    '''计算问句和库中问句的相似度，对候选结果进行二次筛选'''
    def similarity_cosine(self, vector1, vector2):
        cos1 = np.sum(vector1*vector2)
        cos21 = np.sqrt(vector1**2)
        cos22 = np.sqrt(vector2**2)
    ...
    def search_main(self, question):
        ...
        question_vector = self.rep_sentencevector(question, "noseg")
        answer_dict = {}
        for index, candi in enumerate(candi_answers):
            candi_question = candi['sim_question']
            score = candi["score"]/100
            candi_vector = self.rep_sentencevector(candi_question, "noseg")
